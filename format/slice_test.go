...
func TestSlice(t *testing.T) {
	rec := format.NewRecording(
		...
		[]format.Recording{
			format.NewRecording("", "", []format.CaptureCollection{
				position.NewCollection("t", []position.Capture{
					position.NewCapture(1, 2, 3, 4),
					position.NewCapture(3, 4, 5, 6),
					position.NewCapture(4, 5, 6, 7),
	...
	assert.Equal(t, "dum name", recSliced.Name())
	...
	assert.Len(t, recSliced.Recordings()[0].CaptureCollections(), 1)
	assert.Len(t, recSliced.Recordings()[0].CaptureCollections()[0].Captures(), 2)
	assert.Equal(t, 3.0, recSliced.Recordings()[0].CaptureCollections()[0].Captures()[0].Time())
	assert.Equal(t, 4.0, recSliced.Recordings()[0].CaptureCollections()[0].Captures()[1].Time())
}
